...
void exit_sig_handler(int signum)
{
    RCLCPP_INFO(node->get_logger(), "[test_xarm_planner_node_pose] Ctrl-C caught, exit process...");
    rclcpp::shutdown();
    exit(-1);
}
...
public:
        ...
        : Node("pose_subscriber")
    {
        subscription_ = this->create_subscription<geometry_msgs::msg::Pose>(
            "pose_topic", 10, std::bind(&PoseSubscriber::pose_callback, this, std::placeholders::_1));
    }

private:
    void pose_callback(const geometry_msgs::msg::Pose::SharedPtr msg)
    {
        // Assuming your planning and execution code goes here
        // You can use 'msg' to get the target pose information
        RCLCPP_INFO(this->get_logger(), "Received Pose Message: x=%f, y=%f, z=%f", msg->position.x, msg->position.y, msg->position.z);

        // Example: Update target_pose1 with the received pose
        target_pose1 = *msg;
    }

    rclcpp::Subscription<geometry_msgs::msg::Pose>::SharedPtr subscription_;
    geometry_msgs::msg::Pose target_pose1;  // Declare target_pose1 as a member variable
};
...
int main(int argc, char **argv)
{
    ...
    rclcpp::Client<xarm_msgs::srv::PlanPose>::SharedPtr pose_plan_client_ =
        node->create_client<xarm_msgs::srv::PlanPose>("xarm_pose_plan");
    rclcpp::Client<xarm_msgs::srv::PlanExec>::SharedPtr exec_plan_client_ =
        node->create_client<xarm_msgs::srv::PlanExec>("xarm_exec_plan");
    ...
    while (rclcpp::ok())
    {
        // Call services for planning and executing motions
        pose_plan_req->target = pose_subscriber.target_pose1;
        call_request(pose_plan_client_, pose_plan_req);
        call_request(exec_plan_client_, exec_plan_req);
